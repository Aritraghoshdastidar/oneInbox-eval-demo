"""DPO Experiment V4A: Controlled Trajectory Preference Dataset Balancing.

Fine-tunes Qwen2.5-0.5B-Instruct using Direct Preference Optimization (DPO)
on a balanced trajectory preference dataset with paired anti-hesitation negative controls.
...
def run_dpo_training_v4a(
    train_file: str = "data/preferences/dpo_train_formatted_v4a.jsonl",
    val_file: str = "data/preferences/dpo_val_formatted_v4a.jsonl",
    base_model_name: str = "Qwen/Qwen2.5-0.5B-Instruct",
    output_dir: str = "experiments/dpo_qwen05b_v4a",
    lora_r: int = 16,
    lora_alpha: int = 32,
    ...
    print("=" * 70, flush=True)
    print("STARTING DPO EXPERIMENT V4A: BALANCED TRAJECTORY PREFERENCE LEARNING", flush=True)
    print("=" * 70, flush=True)
    ...
    # 2. Prepare Datasets
    print("\n--- [2/6] Loading Formatted Datasets for DPO V4A ---", flush=True)
    ...
    # Strict holdout check before training
    ...
    # 6. Train
    print("\n--- [6/6] Executing DPO V4A Training ---", flush=True)
    ...
    experiment_metadata = {
        ...
        "dataset": {
            ...
            "holdout_verified": "RE-004" not in train_sids and "AMB-001" not in train_sids,
        },
        "hyperparameters": {
            "lora_r": lora_r,
            "lora_alpha": lora_alpha,
...
- **Validation Samples**: {len(val_records)} trajectory pairs (Strict Holdout: RE-004, AMB-001)
- **Epochs**: {num_epochs}
- **Beta**: {beta}
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train DPO V4A LoRA Adapter")
    parser.add_argument("--train_file", default="data/preferences/dpo_train_formatted_v4a.jsonl")
    parser.add_argument("--val_file", default="data/preferences/dpo_val_formatted_v4a.jsonl")
    parser.add_argument("--base_model", default="Qwen/Qwen2.5-0.5B-Instruct")
    parser.add_argument("--output_dir", default="experiments/dpo_qwen05b_v4a")
    parser.add_argument("--epochs", type=int, default=3)
    parser.add_argument("--lr", type=float, default=5e-5)
    parser.add_argument("--beta", type=float, default=0.1)
    args = parser.parse_args()

    run_dpo_training_v4a(
